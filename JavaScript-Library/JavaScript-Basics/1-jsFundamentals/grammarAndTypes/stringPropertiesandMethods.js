/*
-Properties are qualities associated with a data type
-Strings only have one singular property for us to access, that being the length property

*/
//will clarify how many letters does the name have
let myName= "Donovan";
console.log(myName.length);

//will get you number
console.log(typeof myName.length);

/*
Methods are tools that can help us manipulate data
- .property vs .method() 
Methods are functions associated with data types
*/
//will put everything on caps
let myNameIs= "Donovan";
console.log(myNameIs.toUpperCase() );

//we get a booleon as true because it has the same capitalization and same everything,
//if you put it in miniscule letters it will create a false 
let home= "My home is Whitestown";
console.log(home.includes("Whitestown"))

// Challenge" Use google, find MDN documentation for Strings, research and use the split method
//to get an array back from this string

let sent= 'This sentence will be split into individual parts';
console.log(sent.split(" "));


let splitString=sent.split("");
console.log(splitString);